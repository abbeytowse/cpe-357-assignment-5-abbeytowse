...
void send_request(int fd)
{
   ...
   while ((num = getline(&line, &size, stdin)) >= 0) { 
      printf("top of big while loop\n"); 
      ssize_t val; 
      size_t size2; 
      char *line2 = NULL; 
      write(fd, line, num);
      printf("writing to server...\n"); 
      //while ((val = getline(&line2, &size2, network)) > -1) { 
	   //printf("inside inner while loop...\n"); 
	   //printf("%s", line2); 
          //printf("retrieved line of length: %li\n", val); 
      //}
     //printf("exit little while loop\n");  
   } 
   printf("exit while loop client\n");  
